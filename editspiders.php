...
if (empty($r['action'])) {
	$headers = array(
		"id" => array("caption" => "Name", "align" => "center", "color" => 1),
	...
	while ($sp = $sql->fetch($spReq)) {
		$actions = array(
			array('title' => 'Edit', 'href' => 'editspiders.php?action=edit&id='.$sp['bot_name']),
			array('title' => 'Delete', 'href' => 'editspiders.php?action=del&id='.$sp['bot_name'], 'confirm' => true)
		);
		$data[] = array(
			"id" => $sp['bot_name'],
	...
	$pagebar['actions'] = array(array('title' => 'New Spider', 'href' => 'editspiders.php?action=new'));
	RenderPageBar($pagebar);
	RenderTable($data, $headers);
} elseif ($r['action'] == "edit" || $r['action'] == "new") {
	if (!empty($r['act'])) {
		$s = request_variables(array('bot_name','bot_agent'));
		if ($r['action'] == "edit" && $id) {
			if (empty($s['bot_name']) || empty($s['bot_agent'])) {
				$pagebar['message'] = "The name and/or agent for this spider cannot be empty.";
			} else {
				if ($sql->prepare('UPDATE robots SET bot_agent=?, bot_name=? 
							WHERE bot_name=?;', array($s['bot_agent'], $s['bot_name'], $id))) {
			...
			if (empty($s['bot_name']) || empty($s['bot_agent'])) {
				...
				if ($sql->prepare('INSERT INTO robots SET bot_agent=?, bot_name=? ;', 
							array($s['bot_agent'], $s['bot_name']))) {
					$r['action'] = "edit";
	...
	$pagebar['breadcrumb'] = array(array('title' => 'Edit Spider','href' => 'editspiders.php'));
	if ($id) {
		$t = $sql->fetchp('SELECT * FROM robots WHERE bot_name=?', array($id));
		if (!$t && !$_POST['bot_name']) { 
			noticemsg("Notice", "Invalid spider name"); pagefooter(); die();
		} else {
			$pagebar['title'] = $t['bot_name'];
			$pagebar['actions'] = array(array('title' => 'Delete Spider',
											'href' => 'editspiders.php?action=del&id='.$id, 
											'confirm' => true));
		}
	} else {
