...
if (empty($r['action'])) {
	$headers = array(
		"id" => array("caption" => "Text","width" => "32px","align" => "center","color" => 1),
	...
	while ($sm = $sql->fetch($smReq)) {
		...
		$pic = $pics[0];
		$actions = array(
			array('title' => 'Edit','href' => 'editsmilies.php?action=edit&id='.$sm['text']),
			array('title' => 'Delete','href' => 'editsmilies.php?action=del&id='.$sm['text'],'confirm' => true)
		);
		$data[] = array(
			"id" => $sm['text'],
	...
	$pagebar['actions'] = array(array('title' => 'New Smiley','href' => 'editsmilies.php?action=new'));
	RenderPageBar($pagebar);
	RenderTable($data, $headers);
} elseif ($r['action'] == "edit" || $r['action'] == "new") {
	if (!empty($r['act'])) {
		$s = request_variables(array('url','text'));
		if ($r['action'] == "edit" && $id) {
			if (empty($s['text']) || empty($s['url'])) {
				$pagebar['message'] = "The code and/or url for this smiley cannot be empty.";
			} else {
				if ($sql->prepare('UPDATE smilies SET url=?, text=? WHERE text=?;',array($s['url'],$s['text'],$id))) {
			...
			if (empty($s['text']) || empty($s['url'])) {
				...
				if ($sql->prepare('INSERT INTO smilies SET url=?, text=? ;',array($s['url'],$s['text']))) {
					$r['action'] = "edit";
	...
	$pagebar['breadcrumb'] = array(array('title' => 'Edit Smiley','href' => 'editsmilies.php'));
	if ($id) {
		$t = $sql->fetchp('SELECT * FROM smilies WHERE text=?',array($id));
		if (!$t && !$_POST['text']) {
			noticemsg("Notice", "Invalid smiley code"); pagefooter(); die();
		} else {
			$pagebar['title'] = $t['text'].' Smiley';
			$pagebar['actions'] = array(
				array('title' => 'Delete Smiley','href' => 'editsmilies.php?action=del&id='.$id,'confirm' => true)
			);
		}
	} else {
	...
	RenderPageBar($pagebar);
	if (!isset($t['text'])) $t['text'] = '';
	if (!isset($t['url'])) $t['url'] = '';
	$form = array(
		'action' => urlcreate('editsmilies.php', array(
			'action' => $r['action'],
			'id' => $t['text']
		)),
		'method' => 'POST',
