...
" . "<tr><td class=\"b n1\" align=\"left\">The item has been unequipped and sold.</td></tr></table></div>";
	} elseif ($_COOKIE['pstbon'] == - 2) {
		$rdmsg .= "Item Bought<div style=\"float: right\"><a style=\"cursor: pointer;\" onclick=\"dismiss()\">[x]</a></td></tr>
" . "<tr><td class=\"b n1\" align=\"left\">The item has been bought and equipped!</td></tr></table></div>";
	}
}
...
if (!has_perm('use-item-shop')) {
	...
	$user = $sql->fetchq('SELECT u.name, u.posts, u.regdate, r.* ' . 'FROM users u ' . 'LEFT JOIN usersrpg r ON u.id=r.id ' . "WHERE u.id=".$loguser['id']);
	$p = $user['posts'];
...
";
		...
		case 'desc':
			...
			pageheader('Item shop');
			echo  "<style>.disabled{color:#888888} .higher{color:#abaffe} .equal{color:#ffea60} .lower{color:#ca8765}</style>";
			$statlist = '';
			$stathdr = '';
			for ($i = 0; $i < 9; $i++) {
				$st = $item["s$stat[$i]"];
...
";
			...
			while ($item = $sql->fetch($items)) {
				...
				if ($item['id'] && $item['id'] == $eq['e'])
					$comm = $sell;
				elseif ($item['id'] && $item['coins'] <= $GP && $item['coins2'] <= 0)
					$comm = "$buy | $preview";
				elseif (! $item['id'] && ! $eq['e'])
					$comm = '-';
				else
