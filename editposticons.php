...
if ($r['action'] == "del") {
	...
	if ($id > 0) {
		...
		if (!$posticon)
			$pagebar['message'] = "Unable to delete post icon: invalid post icon ID.";
...
if (empty($r['action'])) {
	...
	while ($pi = $sql->fetch($piReq)) {
		...
		$pic = $pics[0];
		$actions = array(
			array('title' => 'Edit','href' => 'editposticons.php?action=edit&id='.$pi['id']),
			array('title' => 'Delete','href' => 'editposticons.php?action=del&id='.$pi['id'], 'confirm' => true)
		);
		$data[] = array(
			"id" => $pi['id'],
	...
	$pagebar['actions'] = array(array('title' => 'New Post Icon','href' => 'editposticons.php?action=new'));
	RenderPageBar($pagebar);
	RenderTable($data, $headers);
} elseif ($r['action'] == "edit" || $r['action'] == "new") {
	if (!empty($r['act'])) {
		$s = request_variables(array('url'));
		if ($r['action'] == "edit" && $id > 0) {
			if ($sql->prepare('UPDATE posticons SET url=? WHERE id=?;',array($s['url'],$id))) {
				$pagebar['message'] = "Post icon successfully updated.";
			} else {
				$pagebar['message'] = "Unable to update Post icon.";
			}
		} elseif ($r['action'] == "new") {
			if ($sql->prepare('INSERT INTO posticons SET url=? ;', array($s['url']))) {
				$id = $sql->insertid();
				$r['action'] = "edit";
	...
	$pagebar['breadcrumb'] = array(array('title' => 'Edit Post Icon','href' => 'editposticons.php'));
	if ($id > 0) {
		$t = $sql->fetchp('SELECT * FROM posticons WHERE id=?',array($id));
		if (!$t) {
			noticemsg("Error", "Invalid post icon ID"); pagefooter(); die();
		} else {
			$pagebar['title'] = 'Post Icon ID '.$t['id'];
			$pagebar['actions'] = array(
				array('title' => 'Delete Post Icon','href' => 'editposticons.php?action=del&id='.$id,'confirm' => true),
			);
		}
	} else {
	...
	$form = array(
		'action' => urlcreate('editposticons.php', array(
			'action' => $r['action'],
			'id' => $t['id']
		)),
		'method' => 'POST',
