...
if(empty($r['action'])) {
	$headers = array(
		"id" => array("caption" => "#","width" => "32px","align" => "center","color" => 1),
	...
	while ($event = $sql->fetch($eventReq)) {
		...
		$actions = array(
			array('title' => 'Edit','href' => 'editevents.php?action=edit&id='.$event['id']),
			array('title' => 'Delete','href' => 'editevents.php?action=del&id='.$event['id'],'confirm' => true)
		);
		$data[] = array(
			"id" => $event['id'],
	...
	$pagebar['actions'] = array(array('title' => 'New Event','href' => 'editevents.php?action=new'));
	RenderPageBar($pagebar);
	RenderTable($data, $headers);
} elseif ($r['action'] == "edit" || $r['action'] == "new") {
	if (!empty($r['act'])) {
		$e = request_variables(array('month','day','year','user','private','event_title'));
		$e['user'] = $_POST['user'];
		if ($r['action']=="edit" && $id > 0) {
			if ($sql->prepare('UPDATE events SET month=?,day=?,year=?,user=?,private=?,event_title=? WHERE id=?;',
					array($e['month'],$e['day'],$e['year'],$e['user'],$e['private'],$e['event_title'],$id))) {
				$pagebar['message'] = "Event successfully updated.";
			} else {
				$pagebar['message'] = "Unable to update event.";
			}
		} elseif ($r['action']=="new"){
			if ($sql->prepare('INSERT INTO events SET month=?,day=?,year=?,user=?,private=?,event_title=? ;',
					array($e['month'],$e['day'],$e['year'],$e['user'],$e['private'],$e['event_title']))) {
				$id = $sql->insertid();
				$r['action'] = "edit";
	...
	$pagebar['breadcrumb'] = array(array('title' => 'Edit Events','href' => 'editevents.php'));
	if ($id > 0) {
		$t = $sql->fetchp('SELECT * FROM events WHERE id=?',array($id));
		if (!$t) {
			noticemsg("Error", "Invalid event ID"); pagefooter(); die();
		} else {
			$pagebar['title'] = $t['event_title'];
			$pagebar['actions'] = array(array('title' => 'Delete Event','href' => 'editevents.php?action=del&id='.$id,'confirm' => true));
		}
	} else {
		$pagebar['title'] = 'New Event';
		$t = array('id' => 0,'event_title' => '','user' => 1,'private' => '0','month' => '','day' => '','year' => '');
	}
	RenderPageBar($pagebar);
	$form = array(
		'action' => urlcreate('editevents.php', array(
			'action' => $r['action'],
			'id' => $t['id']
		)),
		'method' => 'POST',
