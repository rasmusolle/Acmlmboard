...
if (isset($_POST['id']) && $_POST['a'] == 'Save') {
	if (is_numeric($_POST['id'])) {
		...
		if ($fname['size'] > 0) {
			...
			if ($ava_out == 'OK!') {
				if ($_POST['id'] != -1) {
					$sql->query("REPLACE INTO `mood` VALUES (".$_POST['id'].",".$edid.",'".addslashes($_POST['label'])."',1,'')");
				}
			} else { $err .= $ava_out; }
			...
			if (strlen($_POST['url']) > 0) {
				...
				if (!in_array(str_replace("image/","",$img_data['mime']),$ftypes)) {
					$err .= 'Image linked is not a gif, jpg or png file.<br>';
				}
				if (!$err) {
					$sql->query("REPLACE INTO `mood` VALUES (".$_POST['id'].",".$edid.",'".addslashes($_POST['label'])."',0,'".addslashes($_POST['url'])."')");
				}
...
pageheader('Edit Mood Avatars');
//Various magic
if (isset($err)) { noticemsg('Notice', $err); }
?><script>
...
function del(av_id, av_lab) {
	if (confirm("Are you sure you wish to delete "+av_lab+"?")) {
		...
		<?php if (!isset($lnkex)) $lnkex = ''; ?>
		y.open('POST','mood.php<?php echo $lnkex; ?>',true);
...
?>
<div style="margin:4px;float:left;display:inline-block;">
	<table class="c1">
		<tr class="h"><td class="b h">Default</td></tr>
		<tr>
    		<td class="b n2">
    			<div style="padding:0px;margin:0px;width:180px;height:180px;background:url($aurl) no-repeat center;" id="defava"></div>
  			</td>
  		</tr>
  		<tr><td class="b n1"><a href="#" onclick="edit(-1,'','')">Edit</a> | <a href="#" onclick="del(-1,'Default')">Delete</a></td></tr>
	</table>
</div>
<?php
...
for ($i = 1; $mav = $sql->fetch($avas); $i++) {
	...
	?><div style="margin:4px;float:left;display:inline-block;" id="mood<?php echo $mav['id']; ?>">
		<table class="c1">
			<tr class="h"><td class="b h"><?php echo stripslashes($mav['label']); ?></td></tr>
			<tr>
				<td class="b n2">
					<div style="padding:0px;margin:0px;width:180px;height:180px;background:url(<?php echo $aurl; ?>) no-repeat center;"></div>
				</td>
			</tr>
			<tr>
				<td class="b n1">
					<a href="#" onclick="edit(<?php echo $mav['id']; ?>,'<?php echo htmlspecialchars($mav['label']); ?>', '<?php echo $mav['url']; ?>')">Edit</a> | <a href="#" onclick="del(<?php echo $mav['id']; ?>,'<?php echo htmlspecialchars($mav['label']); ?>')">Delete</a>
				</td>
			</tr>
		</table>
	</div><?php
...
if ($fid <= 64) {
	?><div style="margin:4px;float:left;display:inline-block;" id="mood64">
	<table class="c1">
		<tr class="h"><td class="b h" style="width:180px;">&nbsp;</td></tr>
		<tr><td class="b n1"><a href="#" onclick="edit(<?php echo $fid; ?>,'(Label)', '')">Add New</a></td></tr>
	</table></div><?php
}
?>
<br clear="all"><div id="editpane" style="display:none;"><form id="f" action="mood.php<?php echo $lnkex; ?>" enctype="multipart/form-data" method="post">
	<table class="c1">
		<tr class="h"><td class="b h" colspan="2">Editing mood avatar</td></tr>
		<tr id="em">
