...
if (empty($r['action'])) {
	...
	while ($bdg = $sql->fetch($bdgReq)) {
		...
		$actions = array(
			array(
				'title' => 'Edit',
				'href' => 'assignbadges.php?action=edit&uid=' . $uid . '&id=' . $bdg['id']
			),
			array(
				'title' => 'Delete',
				'href' => 'assignbadges.php?action=del&uid=' . $uid . '&id=' . $bdg['id'],
				confirm => true
			)
		);
	...
	if (! empty($r['act'])) {
		$s = request_variables(array(
			'badge_id',
			'badge_var'
		));
		
		if ($r['action'] == "edit" && $id > 0) {
			if ($sql->prepare('UPDATE user_badges SET badge_id=?,badge_var=?,user_id=? WHERE id=?;', array($s['badge_id'],$s['badge_var'],$uid,$id))) {
		...
		elseif ($r['action'] == "new") {
			if ($sql->prepare('INSERT INTO user_badges SET user_id=?,badge_id=?,badge_var=? ;', array($uid, $s['badge_id'], $s['badge_var']))) {
				$id = $sql->insertid();
				$r['action'] = "edit";
	...
	$pagebar['breadcrumb'] = array(
		array(
			'title' => 'Edit Assigned Badges',
			'href' => 'assignbadges.php?uid=' . $uid
		)
	);
	
	if ($id > 0) {
		$t = $sql->fetchp('SELECT * FROM user_badges WHERE id=?', array(
			$id
		));
		...
		$pagebar['actions'] = array(
			array(
				'title' => 'Delete Badge',
				'href' => 'assignbadges.php?action=del&uid=' . $uid . '&id=' . $id,
				'confirm' => true
			)
		);
	} else {
		$pagebar['title'] = 'Assign Badge';		
		$t = array('badge_id' => '', 'badge_var' => '');
	}
	RenderPageBar($pagebar);
	...
	$form = array(
		'action' => urlcreate('assignbadges.php', array(
			'action' => $r['action'],
			'uid' => $r['uid'],
			'id' => $t['id']
		)),
