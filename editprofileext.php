...
$r['id'] = (!isset($r['id']) ? null : $r['id']);
$r['action'] = (!isset($r['action']) ? null : $r['action']);
...
if ($r['action'] == "del") {
	...
	if (!$id)
		$pagebar['message'] = "Unable to delete extended profile field: invalid extended profile field ID.";
...
if(empty($r['action'])) {
	$headers = array(
		"id" => array("caption" => "ID","width" => "32px","align" => "center","color" => 1),
	...
	while ($exf = $sql->fetch($exfReq)) {
		$actions = array(
			array('title' => 'Edit','href' => 'editprofileext.php?action=edit&id='.$exf['id']),
			array('title' => 'Delete','href' => 'editprofileext.php?action=del&id='.$exf['id'],'confirm' => true)
		);
		$format = array("%%%VAL%%%","<",">");
	...
	$pagebar['actions'] = array(array('title' => 'New Extended Profile Field','href' => 'editprofileext.php?action=new'));
	RenderPageBar($pagebar);
	RenderTable($data, $headers);
} elseif ($r['action'] == "edit" || $r['action'] == "new") {
	if (!empty($r['act'])) {
		$s = request_variables(array('id','title','sortorder','fmt','description','icon','validation','example','extrafield','parser'));
		if ($r['action'] == "edit" && $id) {
			if (empty($s['id']) || empty($s['title']) || empty($s['fmt']) || empty($s['description']) || empty($s['validation']) || empty($s['example'])) {
				...
				if ($sql->prepare('UPDATE profileext SET 
					id=?, title=?, sortorder=?, fmt=?, description=?, icon=?, validation=?, example=?, extrafield=?, parser=? WHERE id=?;',
					array($s['id'],$s['title'],$s['sortorder'],$s['fmt'],$s['description'],$s['icon'],$s['validation'],$s['example'],$s['extrafield'],$s['parser'],$id))) {
			...
			if (empty($s['id']) || empty($s['title']) || empty($s['fmt']) || empty($s['description']) || empty($s['validation']) || empty($s['example'])) {
				...
				if($sql->prepare('INSERT INTO profileext SET 
						id=?, title=?, sortorder=?, fmt=?, description=?, icon=?, validation=?, example=?, extrafield=?, parser=? ;',
						array($s['id'],$s['title'],$s['sortorder'],$s['fmt'],$s['description'],$s['icon'],$s['validation'],$s['example'],$s['extrafield'],$s['parser']))) {
					$r['action'] = "edit";
	...
	$pagebar['breadcrumb'] = array(array('title' => 'Edit Extended Profile Field','href' => 'editprofileext.php'));
	if ($id) {
		...
		if (!$t && !$_POST['id']) { noticemsg("Notice", "Invalid extended profile field ID"); pagefooter(); die();
			...
			$pagebar['actions'] = array(array('title' => 'Delete Extended Profile Field','href' => 'editprofileext.php?action=del&id='.$id,'confirm' => true));
		}
	} else {
	...
	if (!isset($t['sortorder'])) $t['sortorder'] = '';
	if (!isset($t['fmt'])) $t['fmt'] = '';
	if (!isset($t['description'])) $t['description'] = '';
	if (!isset($t['icon'])) $t['icon'] = '';
	if (!isset($t['validation'])) $t['validation'] = '';
	if (!isset($t['example'])) $t['example'] = '';
	if (!isset($t['extrafield'])) $t['extrafield'] = '';
	if (!isset($t['parser'])) $t['parser'] = '';
	$form = array(
		'action' => urlcreate('editprofileext.php', array(
			'action' => $r['action'],
			'id' => $t['id']
		)),
		'method' => 'POST',
